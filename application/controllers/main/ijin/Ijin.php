...
class ijin extends CI_Controller {
	...
	public function index(){
		...
		if($this->session->userdata('sts_login') != true){
			...
			$data['title'] = 'ijin';
			$data['sub_title'] = 'List Ijin';
			$data['breadcrumb'] = '
				<li><a href="#"><i class="fa fa-dashboard"></i> Main</a></li>
        		<li class="active">Ijin</li>
			';
			$data['pegawai'] = $this->crud->selectAllOrderby('ijin','id','asc')->result();
			$this->template->view('ijin/index',$data);
		}
	}
	...
	public function tambah(){
		...
		if($this->session->userdata('sts_login') != true){
			...
			$data['title'] = 'Daftar Ijin';
			$data['sub_title'] = 'Input Ijin';
			$data['breadcrumb'] = '
				<li><a href="#"><i class="fa fa-dashboard"></i> Main</a></li>
        		<li class="active">Insert Ijin</li>
			';
			$this->template->view('ijin/tambah',$data);
		}
	}

	public function listPegawai(){
		$sql_listkatagori = '
			SELECT DISTINCT a.ijin AS ijin
			FROM ijin
			ORDER BY a.pegawai ASC
		';
		$data['listPegawai'] = $this->crud->getDataQuery($sql_listijin)->result();
		$data['crud'] = $this->crud;
		$this->load->view('views/ijin/index',$data);
	}
	...
	public function saveData(){
		...
		$form->set_rules('dinas','<b class="text-uppercase">DINAS</b>','required');
		$form->set_rules('durasi','<b class="text-uppercase">DURASI/b','required');
		$form->set_rules('hari','<b class="text-uppercase">HARI/b','required');
		$form->set_rules('alesan','<b>ALASAN</b>','required');
		if($form->run() == FALSE){
			$respon = array(
				'code'=>1,
			...
			$invoice = array(
				...
				'dinas'=>$input->post('dinas'),
				'durasi'=>$input->post('durasi'),
				'hari'=>$input->post('hari'),
				'alasan'=>$input->post('alasan')
			);
			//$d_invoice = $input->post('pegawai');

			$respon = $this->crud->insertDataSave('ijin',$invoice);
		}
